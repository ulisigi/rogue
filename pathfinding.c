...
int checkPositions(int y, int x)
{
...
{
	if (y > 0 && cameFrom[y - 1][x][0] < 0 && checkPositions(y - 1, x))
	{
	...
	if (y < (MAX_HEIGHT - 1) && cameFrom[y + 1][x][0] < 0 && checkPositions(y + 1, x))
	{
		addPositionYX(frontier, frontierCount, y + 1, x);
		frontierCount++;
		cameFrom[y + 1][x][0] = y;
	...
	if (x < (MAX_WIDTH - 1) && cameFrom[y][x + 1][0] < 0 && checkPositions(y, x + 1))
	{
	...
	if (x > 0 && cameFrom[y][x - 1][0] < 0 && checkPositions(y, x - 1))
	{
		addPositionYX(frontier, frontierCount, y, x - 1);
		frontierCount++;
		cameFrom[y][x - 1][0] = y;
